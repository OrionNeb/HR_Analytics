...
library(vcd)
#Paket für die Visualisierung von kategorialen Daten, wie etwa das Mosaikdiagramm
rm(list = ls())
...
```{r}
# Kreuztabelle
table <- table(HR_Analytics_selected$WorkLifeBalance, HR_Analytics_selected$JobSatisfaction)
print(table)
...
mosaic(cross_tab, main = "Mosaikdiagramm von Jobzufriedenheit und Work-Life-Balance")
...
Im Allgemeinen zeigt die Tabelle und das Mosaikdiagramm, dass eine bessere Work-Life-Balance tendenziell mit höherer Jobzufriedenheit assoziiert ist, während eine schlechtere Work-Life-Balance möglicherweise negativ mit der Jobzufriedenheit korreliert. Diese Beobachtungen könnten für weitere statistische Analysen nützlich sein, um die Beziehung zwischen diesen beiden Variablen besser zu verstehen.
...
```{r}
# Erstellen einer Kreuztabelle zwischen JobSatisfaction und WorkLifeBalance
cross_tab <- with(HR_Analytics_selected, table(JobSatisfaction, WorkLifeBalance))
...
Nach Durchführung eines Chi-Quadrat-Tests zur Untersuchung des Zusammenhangs zwischen der Jobzufriedenheit und der Work-Life-Balance ergab sich ein Chi-Quadrat-Wert von 6.5673 bei 9 Freiheitsgraden. Der errechnete p-Wert beträgt 0.6821, was bedeutet, dass es keinen statistisch signifikanten Zusammenhang zwischen den beiden Variablen gibt. In anderen Worten, die Work-Life-Balance scheint die Jobzufriedenheit der Mitarbeiter in dieser Stichprobe nicht signifikant zu beeinflussen.  
...
```{r}
cross_tab <- table(HR_Analytics_selected$JobInvolvement, HR_Analytics_selected$JobSatisfaction)
